...
object AndersonDarlingMTA {
  def main(args: Array[String]): Unit = {
    ...
    ).sortBy(x => x.timestamp.getMillis ) // go ahead and sort now


    val stops = train6Down.map(_.stop).distinct.take(3)
    val results = stops.map(stop => Try(testExpDistPerStop(train6Down, stop)))
  }
